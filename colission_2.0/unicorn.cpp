...
void unicorn::move(sf::Vector2f delta) {
        ...
        if(delta.x < 0 && !checker_L.L){//nog niet getest
            going_left = true;
        ...
        if (checker_R.R){
                float x = checker_R.the_object->get_position().x-size.x;
                if (position.x != x){
                    position.x  = x;
                    unicorn_animation.set_position(position);
        ...
        if (checker_L.L){
                float x = checker_L.the_object->get_position().x+checker_L.the_object->get_size().x;
                 if (position.x != x){
                    position.x  = x;
                    unicorn_animation.set_position(position);
                 }
